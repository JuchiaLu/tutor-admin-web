    ...
    <el-table ref="multipleTable" :data="records" style="width: 100%" height="550" border fit stripe row-key="id"
      ...
      <el-table-column label="类型" >
        <template slot-scope="scope">
          <el-tag  effect="dark" v-if="scope.row.type===1">站内跳转</el-tag>
          <el-tag type="success"  effect="dark" v-else>站外跳转</el-tag>
        </template>
      </el-table-column>
          ...
          <el-col :span="12">
            <el-form-item label="类型" prop="type">
              <el-radio-group v-model.number="tempForm.type">
                <el-radio :label="1">站内</el-radio><el-radio :label="2">站外</el-radio>
              </el-radio-group>
            </el-form-item>
          </el-col>
          <el-col :span="12">
...
<script>
  ...
  import * as advertisementApi from '@/api/system/advertisement'
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        ...
        advertisementApi.page(this.listQuery).then(response => {
      ...
      handleDelete(index, row) {
        this.$confirm('此操作将永久删除, 是否继续?', '提示', {
          ...
          return advertisementApi.deleteById(row.id)
        }).then(() => {
          this.$message({
            message: '删除成功',
            type: 'success',
      ...
      handleStatusChange(row){
        advertisementApi.patch(row).then(() => {
      ...
      this.$refs['ruleForm'].validate((valid) => {
        if (valid) {
          advertisementApi.save(this.tempForm).then(response => {
            return response
          }).then((response) => {
            this.$message({
              message: '添加成功',
      ...
      this.$refs['ruleForm'].validate((valid) => {
        if (valid) {
          advertisementApi.patch(this.tempForm).then(respose => {
            return respose
          }).then((response) => {
            this.$message({
              message: '修改成功',
