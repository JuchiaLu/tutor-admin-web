<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      <el-input placeholder="标题 ↵ " v-model="listQuery.name" clearable suffix-icon="el-icon-search"
    ...
    <el-table ref="multipleTable" :data="records" style="width: 100%" height="550" border fit stripe
      ...
      <el-table-column  sortable prop="name" label="标题" ></el-table-column>
      <el-table-column prop="url" label="跳转路径" ></el-table-column>

      <el-table-column label="打开方式" >
        <template slot-scope="scope">
          <el-tag  effect="dark" v-if="scope.row.type===1">本窗口</el-tag>
          <el-tag type="success"  effect="dark" v-else>新窗口</el-tag>
        </template>
      </el-table-column>
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="名字" prop="name">
              <el-input v-model="tempForm.name" clearable></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="12">
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="打开方式" prop="type">
              <el-radio-group v-model.number="tempForm.type">
                <el-radio :label="1">本窗</el-radio><el-radio :label="2">新窗</el-radio>
              </el-radio-group>
            </el-form-item>
          </el-col>
          <el-col :span="12">
...
<script>
  ...
  import * as navigationApi from '@/api/system/navigation'
  ...
  export default {
        ...
        dialogMap: {
          create: {
            ...
            rules: {
              ...
              type: [{required: true, message: '打开方式必填', trigger: 'blur'},],
              url:[ {required: true, message: '跳转地址必填', trigger: 'blur'},],
          ...
          update: {
            ...
            rules: {
              ...
              type: [{required: true, message: '打开方式必填', trigger: 'blur'},],
              url:[ {required: true, message: '名字必填', trigger: 'blur'},],
              status: [{required: true, message: '状态必选', trigger: 'blur'},],
            },
          },
        },
    ...
    methods: {
      ...
      getList() {
        ...
        navigationApi.page(this.listQuery).then(response => {
      ...
      handleDelete(index, row) {
        this.$confirm('此操作将永久删除, 是否继续?', '提示', {
          ...
          return navigationApi.deleteById(row.id)
        }).then(() => {
          this.$message({
            message: '删除成功',
            type: 'success',
      ...
      this.$refs['ruleForm'].validate((valid) => {
        if (valid) {
          navigationApi.save(this.tempForm).then(response => {
            return response
          }).then((response) => {
            this.$message({
              message: '添加成功',
      ...
      this.$refs['ruleForm'].validate((valid) => {
        if (valid) {
          navigationApi.patch(this.tempForm).then(respose => {
            return respose
          }).then((response) => {
            this.$message({
              message: '修改成功',
