<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      ...
      <el-button type="primary" icon="el-icon-plus" @click="uploadDialogVisible=true">上传</el-button>
    </div>
    ...
    <el-table ref="multipleTable" :data="records" style="width: 100%" height="550" border fit stripe row-key="id"
      ...
      <el-table-column prop="size" label="文件大小" >
        <template slot-scope="scope">
          {{Math.floor(scope.row.size/1204)}} KB
        </template>
      </el-table-column>

      <el-table-column prop="type" label="文件后缀" >
        <template slot-scope="scope">
          <el-tag type="info"  effect="dark">{{scope.row.type}}</el-tag>
        </template>
      </el-table-column>

      <el-table-column label="储存类型" >
        <template slot-scope="scope">
          <el-tag  effect="dark" v-if="scope.row.storeType===1">七牛云存储</el-tag>
          <el-tag type="success"  effect="dark" v-else>本地存储</el-tag>
        </template>
      </el-table-column>

      <el-table-column  type="index" label="图片预览" width="240">
        <template slot-scope="scope">
          <el-image
    ...
    <el-dialog :title="dialogMap[dialogStatus].title" :visible.sync="dialogVisible">
      <el-form :model="tempForm" :rules="dialogMap[dialogStatus].rules" ref="ruleForm" label-width="100px" status-icon>
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="后缀" prop="name">
              <el-input v-model="tempForm.type" clearable ></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="12">
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="存储" prop="type">
              <el-radio-group v-model.number="tempForm.storeType">
                <el-radio :label="1">七牛</el-radio><el-radio :label="2">本地</el-radio>
              </el-radio-group>
            </el-form-item>
          </el-col>
          <el-col :span="12">
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        ...
        attachmentApi.page(this.listQuery).then(response => {
      ...
      handleDelete(index, row) {
        this.$confirm('此操作将永久删除, 是否继续?', '提示', {
          ...
          return attachmentApi.deleteById(row.id)
        }).then(() => {
          this.$message({
            message: '删除成功',
            type: 'success',
      ...
      this.$refs['ruleForm'].validate((valid) => {
        if (valid) {
          attachmentApi.save(this.tempForm).then(response => {
            return response
          }).then((response) => {
            this.$message({
              message: '添加成功',
      ...
      this.$refs['ruleForm'].validate((valid) => {
        if (valid) {
          attachmentApi.patch(this.tempForm).then(respose => {
            return respose
          }).then((response) => {
            this.$message({
              message: '修改成功',
