  ...
  export default {
    ...
    data() {
      return {
        ...
        defaultExpandedKeys:['1235138870717915138'],
    ...
    methods: {
      ...
      updateForm(){
        categoryApi.patch(this.tempCategory).then(response=>{
          this.$message({
            message: '修改成功',
            type: 'success',
            duration: 3000
      ...
      loadNode(node, resolve) {
        if (node.level === 0) {
          ...
          categoryApi.listByParentId(pid).then(response => {
            resolve(response)
            return
          }).catch(reason => {
            resolve([])
      ...
      saveForm(){
        categoryApi.save(this.tempCategory).then(response=>{
          this.$message({
            message: '新添成功',
            type: 'success',
            duration: 3000
      ...
      handleDelete() {
        this.$confirm('此操作将永久删除, 是否继续?', '提示', {
          ...
          return categoryApi.deleteById(this.tempCategory.id)
        }).then(() => {
          this.$message({
            message: '删除成功',
            type: 'success',
